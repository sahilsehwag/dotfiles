...
if F_isMac; then
	...
	#F_isSoftlink "~/Library/Application Support/lazygit" || ln -sv $script_directory/ ~/Library/Application\ Support/lazygit

	ln -sv $script_directory/config.yml ~/Library/Application\ Support/lazygit/config.yml
elif isLinux; then
	F_isSoftlink ~/.config/lazygit || ln -sv $script_directory/ ~/.config/lazygit
else
	echo "Unsupported OS"
	exit 1
fi
