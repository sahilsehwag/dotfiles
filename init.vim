			...
			"TODO:PROJECTINATOR
				...
				"DEFAULTS
					if ExistsAndTrue('g:projectinator_enable_default_mappings')
						nnoremap <silent> <Leader>po :call fzf#run(fzf#wrap({
							\ 'source'	: g:jaat_find_directories_command . ' ' . g:jaat_root_path,
							\ 'sink'	: 'cd',
							\ 'options' : '--prompt "open-project> "'
						\}))<CR>

						nnoremap <silent> <Leader>pf :call fzf#run(fzf#wrap({
							\ 'source'	: g:jaat_find_files_command,
							\ 'sink'	: 'edit',
							\ 'options' : '--prompt "open-project-file> "'
						\}))<CR>
						nmap <silent> <M-r> :call fzf#run(fzf#wrap({
							\ 'source'	: g:jaat_find_files_command,
							\ 'sink'	: 'edit',
							\ 'options' : '--prompt "open-project-file> "'
						\}))<CR>

						nnoremap <silent> <Leader>pt :call fzf#run(fzf#wrap({
							\ 'source'	: g:jaat_find_lines_command,
							\ 'sink'	: 'edit',
							\ 'options' : '--prompt "search-project> "'
						\}))<CR>

						nnoremap <silent> <Leader>pe :Vifm .<CR>
						nnoremap <silent> <Leader>pE :Vifm %:p:h<CR>
					endif
			"TODO:PACKMAN
			"TODO:FRAMEWORKS
		"LIBRARIES
		...
		"TODO:FZF
			""TODO:FZF-FD
				"if executable('fd')
					...
					""DEFAULTS
				"else
					"PEchoError('"fd" is not installed')
				"endif
			""TODO:FZF-FIND
				"if executable('find') && !IsWindows()
				"else
					"PEchoError('"find" is not installed')
				"endif
			""TODO:FZF-ACK
				"if executable('ack')
				"else
					"PEchoError('"ack" is not installed')
				"endif
			""TODO:FZF-RIPGREP
				"if executable('rg')
				"else
					"PEchoError('"rg" is not installed')
				"endif
			""TODO:FZF-AG
				"if executable('ag')
				"else
					"PEchoError('"ag" is not installed')
				"endif
			"TODO:FZF-MENU
				"USE:FZF|FZF_PREFIX
...
"MAPPINGS
	...
	"LEADER
		"BASICS
			"TODO:FIX
				execute 'nnoremap <silent> <' . g:modifier_leader . '-space> <ESC>'
		"EDITING
			"OPERATORS
			"OBJECTS
			"MOTIONS
		...
		"INTERFACE
			...
			"BUFFERS
				...
				execute 'nmap <silent> <' . g:modifier_leader . '-p> :bprevious<CR>'
				execute 'nmap <silent> <' . g:modifier_leader . '-n> :bnext<CR>'
			"WINDOWS
				...
				if has('nvim')
					execute 'nmap <silent> <' . g:modifier_leader . '-h> <C-w><C-h>'
					execute 'nmap <silent> <' . g:modifier_leader . '-j> <C-w><C-j>'
					execute 'nmap <silent> <' . g:modifier_leader . '-k> <C-w><C-k>'
					execute 'nmap <silent> <' . g:modifier_leader . '-l> <C-w><C-l>'

					execute 'imap <silent> <' . g:modifier_leader . '-h> <ESC><C-w><C-h>'
					execute 'imap <silent> <' . g:modifier_leader . '-j> <ESC><C-w><C-j>'
					execute 'imap <silent> <' . g:modifier_leader . '-k> <ESC><C-w><C-k>'
					execute 'imap <silent> <' . g:modifier_leader . '-l> <ESC><C-w><C-l>'
				endif
			"TERMINAL
				if has('nvim')
					"MAPPINGS
			...
			"SYSTEM
				Plug 'junegunn/fzf', {'dir': '~/.fzf', 'do': { -> fzf#install() }}
					...
					"MAPPINGS
						"SINK:BUFFER
						"SINK:INTERNAL
							nnoremap <silent> <Leader>nf :call fzf#run(fzf#wrap({
								\'source' : g:jaat_find_directories_command . ' ' . g:jaat_drive_path,
								\'sink'   : 'Vifm',
							\}))<CR>
						...
						"SINK:EXTERNAL
							"FILESYSTEM
								if IsNix()
									nnoremap <silent> <Leader>nw  :call fzf#run(fzf#wrap({'source': 'find ~ -type d' , 'sink': 'SaveAs'			 }))<CR>
									nnoremap <silent> <Leader>nW  :call fzf#run(fzf#wrap({'source': 'find ~ -type d' , 'sink': 'SaveAs!'			 }))<CR>
									nnoremap <silent> <Leader>lnf :call fzf#run(fzf#wrap({'source': 'find ~ -type d' , 'sink': 'NewFile'			 }))<CR>
									nnoremap <silent> <Leader>lnd :call fzf#run(fzf#wrap({'source': 'find ~ -type d' , 'sink': 'NewDirectory'	 }))<CR>
									nnoremap <silent> <Leader>ldf :call fzf#run(fzf#wrap({'source': 'find ~ -type f' , 'sink': 'DeleteFile'		 }))<CR>
									nnoremap <silent> <Leader>ldd :call fzf#run(fzf#wrap({'source': 'find ~ -type d' , 'sink': 'DeleteDirectory'  }))<CR>
									nnoremap <silent> <Leader>ldD :call fzf#run(fzf#wrap({'source': 'find ~ -type d' , 'sink': 'DeleteDirectory!' }))<CR>

									nnoremap <silent> <Leader>nr  :call fzf#run(fzf#wrap({'source': 'ag --hidden --ignore .git -g "" ~' , 'sink': 'Read!' }))<CR>
								endif
					"EXTENSIONS
						Plug 'junegunn/fzf.vim'
							"MAPPINGS
								...
								"INBUILT
									...
									"utilsnips
									"nnoremap <silent> <Leader>nS :Snippets<CR>
								"COMPLETION
				...
				Plug 'voldikss/vim-floaterm'
					...
					execute 'tnoremap <silent> <' . g:modifier_leader . '-k> <C-\><C-n>:FloatermKill<CR>'
			...
			"TODO:DEVELOPMENT
				...
				"AUTOFORMAT
					Plug 'chiel92/vim-autoformat'
						...
						"MAPPINGS
							nnoremap <Leader>vtf :call AutoFormatToggle()<CR>
				"SNIPPETS
